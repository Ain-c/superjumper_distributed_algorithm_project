...
public class WorldRenderer {
	...
	private HashMap<String, Position> enemyPosition = new HashMap<String, Position>();
	...
	private void renderEnemy () {
		...
		int i = 0;
		for (Map.Entry<String, Position> entry : enemyPosition.entrySet()) {
			Position p = entry.getValue();
			if (i == 0) {
				batch.draw(Assets.enemyBob1, p.getX(), p.getY(), p.getWidth(), p.getHeight());
				i++;
			} else {
				batch.draw(Assets.enemyBob2, p.getX(), p.getY(), p.getWidth(), p.getHeight());
			}
		}
	...
	public void updateEnemyLocation (String userName, float x, float y, float width, float height) {
		// DATest
		/*
		 * this._x = x; this._y = y; this._width = width; this._height = height;
		 */
		Position p = new Position(x, y, width, height);
		enemyPosition.put(userName, p);
	}
